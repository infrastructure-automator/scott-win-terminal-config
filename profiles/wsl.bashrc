...
## Oh-My-Posh Init
windows_posh_theme_path=$(wslpath "$(powershell.exe -NoProfile '$env:USERPROFILE' | tr -d '\r')/.terminal-config/oh-my-posh" )

function dft {
    echo ${NORMAL_TEXT}"Loading Default Prompt (Type \"${HIGHLIGHT_TEXT}eft${NORMAL_TEXT}\" to switch to Expanded Prompt)"${NORMAL_TEXT}
    eval "$(oh-my-posh init bash --config "${windows_posh_theme_path}/scott-default.omp.json" )"
}

function eft {
    echo ${NORMAL_TEXT}"Loading Expanded Prompt (Type \"${HIGHLIGHT_TEXT}dft${NORMAL_TEXT}\" to switch to Default Prompt)"${NORMAL_TEXT}
    eval "$(oh-my-posh init bash --config "${windows_posh_theme_path}/scott-expanded.omp.json" )"
}
